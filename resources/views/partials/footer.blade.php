<footer>
    <div class="container-fluid p-4 bg-dark text-white">
        <div class="row">
            <div class="col-12 col-md-4 d-flex flex-column align-items-center">
                <div>
                    <h4 class="f-title-primary">Conocénos</h4>
                    <ul class="group-link">
                        <li><a class="link" href="{{ route('main.nosotros') }}">Historia</a></li>
                        <li><a class="link" href="#">Mapa del sitio</a></li>
                    </ul>
                </div>
            </div>
            <div class="col-12 col-md-4 d-flex  flex-column align-items-center">
                <div>
                    <h4 class="f-title-primary">Politicas</h4>
                    <ul class="group-link">
                        <li><a class="link" href="{{ route('politicas.politicas') }}">Políticas
                                Génerales</a></li>
                        <li><a class="link" href="{{ route('politicas.condiciones') }}">Devoluciones y
                                cambios</a></li>



                        <li><a class="link" href="{{ route('politicas.como_comprar') }}">¿Como Comprar?</a>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="col-12 col-md-4 d-flex flex-column align-items-center">
                <div>
                    <h4 class="f-title-primary">Redes Sociales</h4>
                    <ul class="group-link">
                        <li><a class="link" href="">Instagram</a></li>
                        <li><a class="link" href="">Facebook</a></li>
                    </ul>
                </div>
            </div>
        </div>

    </div>
    <div class="bg-light">
        <p class="text-center p-2">Todos los derechos reservados para @Technoly Box</p>
    </div>
</footer>
