                    ...
                    <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        @foreach ($categories as $category)
                            <a class="dropdown-item"
                                href="{{ route('main.productos', $category->name) }}">{{ $category->name }}</a>
                        @endforeach
                    </div>
                </li>
            </ul>
                ...
                @if ($product->stock->status != 0)
                    ...
                    <div class="col-6 col-md-6 col-lg-3">
                        <div class="card border-0 mb-3">
                            ...
                            @endif
                            <div class="m-auto">
                                <img class="card-img w-100" src="{{ asset('storage/' . $product->image) }}" alt="">
                            </div>
                            <div class="card-body">
                            ...
                            <div class="card-footer border-0 d-flex justify-content-between">
                                <form action="{{ route('cart.store') }}" method="POST">
                                    ...
                                    @if ($product->discount > 0)
                                        <input type="hidden"
                                            value="{{ calPercentaje($product->discount, $product->sale_price) }}"
                                            id="price" name="price">
                                    @else
                                    ...
                                    <button {{ $product->stock->stock > 0 ? '' : 'disabled' }}
                                        class="btn btn-outline-secondary btn-sm btn-md" class="tooltip-test"
                                        title="add to cart">
